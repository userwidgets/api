...
export class Users {
	...
	async authenticate(
		...
		if (!current)
			...
			result = gracely.client.unauthorized()
		else if (current.twoFactor && !twoFactorCode)
			result = gracely.client.missingHeader("twoFactor", "Time based code generated by authenticator.")
		else if (current.twoFactor && twoFactorCode && !(await twoFactor.check(current.twoFactor, twoFactorCode)))
